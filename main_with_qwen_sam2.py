...
try:
    # [修改点 1] 导入新的类名 FastSAM2Tracker，并取别名保持代码一致性
...
class VideoThread(QThread):
    ...
    def run(self):
        try:
            ...
            # [修改点 2] 初始化 FastSAM2Tracker (不再需要 interval 参数)
        ...
        while self.running:
            ...
            try:
                ...
                if cmd_type == "detect":
                    ...
                    if self.current_frame is not None:
                        # [修改点 4] 重写手动追踪逻辑，适配 SAM 2
                        # CSRT 已被移除，我们这里通过调用 tracker 内部方法来初始化 SAM 2
                        try:
                            x, y, w, h = cmd_data
                            # 转换为 xyxy
                            ...
                            # 1. 缩放预处理
                            img_small, scale = self.tracker._preprocess(self.current_frame)
                            self.tracker.predictor.set_image(img_small)

                            # 2. 坐标转换并推理
                            box_small = box_xyxy * scale
                            masks, scores, _ = self.tracker.predictor.predict(box=box_small, multimask_output=False)

                            # 3. 设置追踪器状态
                            if scores[0] > 0.0:  # 只要有置信度就启动
                                self.tracker.last_box = self.tracker._mask_to_box(masks[0])
                                self.tracker.scale_factor = scale
                                self.tracker.is_tracking = True
                                self.tracker.target_prompt = "Manual"
                                self.tracker.lost_counter = 0
                                self.status_signal.emit("Manual Tracking (SAM2) Started")
                            else:
                                self.status_signal.emit("Manual Init Failed (Low Conf)")
            ...
            # 1. 追踪更新
            # [修改点 5] 适配新的 update 返回值: is_track, box, mask, info
            # 注意: 新的 box 是 [x1, y1, x2, y2]
            is_tracking, box_xyxy, mask, status_info = self.tracker.update(frame)

            # 转换 box 为 xywh 格式，用于后续的深度计算和 UI 兼容
            bbox_xywh = None
            ...
            if self.depth_estimator:
                current_bbox_for_depth = bbox_xywh if is_tracking else None
                object_distance, _ = self.depth_estimator.estimate_bbox_depth(
                    self.current_frame, current_bbox_for_depth
                )
                if object_distance: print(f"Depth: {object_distance:.2f}")
            ...
            if is_tracking and bbox_xywh:
                ...
                if mask is not None:
                    ...
                    colored_mask[full_mask > 0] = (0, 100, 255)  # 橙色
    ...
    def stop(self):
        ...
        if self.tracker: self.tracker.stop()  # [修改点 6] 改为 stop()
        self.quit()
        self.wait()
...
class App(QMainWindow):
    ...
    def ask_vqa(self):
        ...
        # 注意：tracker.last_box 是 xyxy 且是 numpy array
        # 但我们这里更方便的是取当前渲染用的 bbox_xywh
        # 简单起见，我们重新判断 tracker 状态

        if tracker and tracker.is_tracking and tracker.last_box is not None:
            # last_box 是推理尺寸的，不能直接用，应该用 update 中计算好的 bbox_xywh
            # 但这里不在 update 循环里。
            # 我们需要手动转换一下
            scale = tracker.scale_factor
            if scale > 0:
                x1, y1, x2, y2 = tracker.last_box / scale
                w_box, h_box = x2 - x1, y2 - y1
                x, y = x1, y1

                h_img, w_img = current_img.shape[:2]
                pad = 20
                x1_crop, y1_crop = max(0, int(x - pad)), max(0, int(y - pad))
                x2_crop, y2_crop = min(w_img, int(x + w_box + pad)), min(h_img, int(y + h_box + pad))

                if x2_crop > x1_crop and y2_crop > y1_crop:
                    current_img = current_img[y1_crop:y2_crop, x1_crop:x2_crop]
